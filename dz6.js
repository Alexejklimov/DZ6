...
let ar = [-3, 5, 2, -5]
ar.forEach((x, i, arr) => { if (arr[i] < 0) arr[i] = 0 })
document.write(ar)
...
arrCina.forEach((x, i, arr) => {
    if (arr[i] >= 1000) (arr[i] *= 0.7)
    if (arr[i] < 1000) (arr[i] *= 1.2)
})
...
auto.every(x => {
    if (x > 2000) return ('every cars are hevier than 2t.')
    if (x < 2000) return ('NOT every cars are hevier than 2t.')
})
...
client.some(x => {
    if (x < 10) return ('it was a few days when clients quatity was less than 10')
    if (x < 10) return ('every day clients quatity was more than 10')
})
...
let maxCount = maxArr.reduce(((max, x, i, arr) => x > max ? max = x : max), 0)
...
arrCin.forEach((x, i, arr) => {
    if (arr[i] > 1000) (arr[i] *= 0.8)
    if (arr[i] < 1000) (arr[i] *= 0.95)
})
...
const sumArr = arrSum.reduce(((acc, x) => {
    if (1000 < x < 2000) acc += x
}), 0)
